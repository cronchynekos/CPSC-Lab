...
BookList::BookList(const std::size_t& newSize)
    : _capacity(newSize), _books_array_size(0), _bookArray(new Book[newSize])
{}
...
BookList & BookList::operator+=( const BookList & rhs)
{
  ...
  // If exceeds, then stop adding
    if (this->_books_array_size + rhs._books_array_size < this->_capacity)
    {
        for (unsigned int i = 0; i < rhs._books_array_size; ++i)
        {
            _bookArray[_books_array_size] = rhs[i];
            _books_array_size++;
        }
    }else{
        int difference = this->_capacity - this->_books_array_size;
        for (unsigned int i = 0; i < difference; i++) {
            this->_bookArray[this->_books_array_size] = rhs[i];
            this->_books_array_size++;
        }
    }
    return *this;
}
